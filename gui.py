...
# Helper functions for PX4 states 
...
class RosNodeWorker(QObject):
    ...
    odometry_data_ready = Signal(float, float, float, float) 
    mission_state_ready = Signal(str)
    vehicle_status_ready = Signal(str, str, bool, bool) 
    battery_status_ready = Signal(float, float) 
    fire_target_data_ready = Signal(int, float, float, float, bool) 
    person_safety_alerts_ready = Signal(list) 
    ...
    def __init__(self):
        ...
        # QoS settings for general sensor 
        ...
        # QoS settings for PX4 status messages 
        ...
        # QoS settings for custom mission_state topic
    ...
    def run(self):
        ...
        self.node_logger = self.node.get_logger() 
        ...
        self.node.create_subscription(
            RosString, '/firedronex/gui_info', 
            self.gui_info_callback, self.mission_state_qos 
        )
    ...
    # Function to convert ROS image to QImage
    ...
    # Function to Yolo debug image callback
    ...
    # Function to Depth debug image callback
    ...
    # Function to Raw camera image callback
    ...
    # Function to Person detections callback
    ...
    # Function to process the person focus feed
    def _process_person_focus_feed(self):
        ...
        try:
            ...
            if self.latest_person_detections: 
                ...
                if crop_x2 > crop_x1 and crop_y2 > crop_y1 and box_w > 0 and box_h > 0: 
                    focused_cv_image = cv_image_bgr[crop_y1:crop_y2, crop_x1:crop_x2].copy()

                    # Adjusting person's bounding box coordinates for the cropped image
                    new_px1 = px1 - crop_x1
                    new_py1 = py1 - crop_y1
                    new_px2 = px2 - crop_x1
                    new_py2 = py2 - crop_y1
                    
                    # Drawing the bounding box and label on the FOCUSED image
                    ...
                    # Placing the text inside the bounding box, near top-left
                    text_x = new_px1 + 5 
                    text_y = new_py1 + 15 

                    # Basic checking to keep text within box
                    if new_py1 + 15 > new_py2 - 5 : 
                         text_y = new_py1 + int((new_py2 - new_py1) * 0.5) 
                    if new_px1 + 5 > new_px2 - 5 :
            ...
            # Drawing on the full image if no focused image is created
            if q_img_to_emit is None:
                cv_image_bgr_display = cv_image_bgr.copy() # Drawing on a copy for the full view
                for (x1_full, y1_full, x2_full, y2_full, score_full) in self.latest_person_detections:
                    cv2.rectangle(cv_image_bgr_display, (x1_full, y1_full), (x2_full, y2_full), (255, 0, 0), 2) 
                    label_full = f"Person: {score_full:.2f}"
                    # Standard text placement for full image
            ...
            if self.node_logger: # Checking if logger exists before using it
                self.node_logger.error(f"Error processing person focus feed: {e}", throttle_duration_sec=1)
    
    # Function to Odometry callback
    ...
    # Function to Mission state callback
    ...
    def gui_info_callback(self, msg: RosString):
        ...
        try:
            ...
            person_alerts = [] # Initializing list for person safety alerts
            ...
            if "detections" in data:
                ...
                # Taking the first fire detection if no current target is found
                ...
                # Processing person detection for safety alerts
                for det in data["detections"]:
                    if det.get("object_type") == "person":
                        ...
                        # Adding to alerts if distance is valid and potentially dangerous
                        if alert_data["dist"] != -1.0: 
    ...
    # Function to Vehicle status callback
    ...
    # Function to Battery status callback
    def battery_status_callback(self, msg: BatteryStatus):
        ...
        percentage = msg.remaining * 100.0 if msg.remaining >= 0 else 0.0 
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        mission_group.setStyleSheet(odom_group.styleSheet()) 
        ...
        fire_target_group.setStyleSheet(odom_group.styleSheet()) 
        ...
        self.fire_target_status_label = self._create_status_indicator_label("NO TARGET") 
        ...
        center_data_panel.addWidget(self.person_alerts_group) 
        ...
        self.ros_worker.person_safety_alerts_ready.connect(self.update_person_safety_display) 

        self.ros_thread.started.connect(self.ros_worker.run)
        self.ros_thread.finished.connect(self.ros_worker.deleteLater) 
        self.ros_thread.start()
    
    # Function to create video feed label container
    ...
    # Function to create status indicator label
    ...
    # Function to update image on label
    ...
    def update_mission_state(self, state_str):
        ...
        elif state_str == "CIRCLE": color = "lightblue" 
    ...
    def update_vehicle_status(self, arming_str, nav_str, is_failsafe, preflight_ok):
        ...
        if nav_str == "OFFBOARD": nav_bg_color = "#558B2F" # Darker Green 
    ...
    def update_person_safety_display(self, alerts_data):
        ...
        for alert in alerts_data:
            ...
            if alert['dist'] < 1.5 and alert['dist'] != -1.0:
                ...
                label.setStyleSheet("color: #DDD; padding: 3px;") 
    ...
    def closeEvent(self, event):
        ...
        if self.ros_thread.isRunning():
            ...
            if not self.ros_thread.wait(3000):
...
if __name__ == '__main__':
    ...
    # Getting screen geometry to center splash screen
    ...
    # Processing events to make sure splash screen is displayed
    app.processEvents()    
    start_time = time.time()

    window = MainWindow() # Creating main window

    # Ensuring splash is shown for a few seconds
